...
assert.equal((data.match(/<details class="data-project"/g) || []).length, 2);
for (const id of ["sfsc-data", "kcsc-data"]) {
  assert.match(data, new RegExp(`id="${id}"`));
}
for (const href of [
  "https://sfsc.amyc.us/",
  "https://github.com/aimesy/sfsc",
  "https://github.com/aimesy/sfsc-data",
  "https://aimesy.github.io/kcsc/",
  "https://github.com/aimesy/kcsc",
  "https://github.com/aimesy/kcsc-data",
]) {
  assert.ok(data.includes(`href="${href}"`), `data.html must link to ${href}`);
}
assert.match(data, /assets\/data-index\.css/);
assert.match(data, /assets\/data-index\.js/);
assert.doesNotMatch(data, /<iframe\b/i);
...
console.log("AMYC Data index integration checks passed.");
